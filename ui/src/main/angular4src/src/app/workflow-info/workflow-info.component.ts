...
@Component({
  ...
  styleUrls: ["./workflow-info.component.css"]
})
export class WorkflowInfoComponent implements AfterViewInit, OnDestroy {
  ...
  initialize() {
    ...
    if (!this.Idpdata.workflowTrigger &&
      (this.Idpdata.createWorkflowSequenceflag === undefined || !this.Idpdata.createWorkflowSequenceflag) ) {
      ...
      if (this.Idpdata.workflowTrigger) {
        ...
        this.IDPWorkflowParamData.applicationName =
        this.Idpdata.triggerJobData.hasOwnProperty("applicationName") ? this.Idpdata.triggerJobData.applicationName : "";
        this.IDPWorkflowParamData.pipelineName =
         this.Idpdata.triggerJobData.hasOwnProperty("pipelineName") ? this.Idpdata.triggerJobData.pipelineName : "";
        ...
        console.log("workflow sequence: " + this.Idpdata.triggerWorkflowJobData.pipelines);
      ...
      this.IDPWorkflowParamData.pipelineSelection[i].appSelectionDetails = [];
          for (const appDetails of pipe.applicationDetails) {
            this.IDPWorkflowParamData.pipelineSelection[i].appSelectionDetails.push({});
        this.IDPWorkflowParamData.pipelineSelection[i].appSelectionDetails[j].pipelineSelectionDetails = [];
        for (const pipeDetails of appDetails.pipelineDetails) {
        this.IDPWorkflowParamData.pipelineSelection[i].appSelectionDetails[j].pipelineSelectionDetails.push("on");
        this.workflowData.workflowSequence[i].applicationDetails[j].applicationName = pipeDetails.applicationName;
        if (this.workflowData.workflowSequence[i].applicationDetails[j].pipelineNameTrigger === undefined) {
          this.workflowData.workflowSequence[i].applicationDetails[j].pipelineNameTrigger = pipeDetails.pipelineName;
        } else {
          ...
          this.workflowData.workflowSequenceTemp[i].applicationName = pipeDetails.applicationName;
          this.workflowData.workflowSequenceTemp[i].pipelineName = pipeDetails.pipelineName;
          this.workflowData.workflowSequenceTemp[i].appPipeNamesDetails = true;
        }
        j++;
    }
    i++;
        }
      } else if (this.formStatusObject.operation === "copy" || this.formStatusObject.operation === "edit") {
        this.newOrEditWorkflow = "old";
        // edit condition
        console.log("copy Edit:");
        console.log(this.workflowData.workflowSequence);
        if (this.resetFlag) {
          try {
            this.workflowData.workflowSequence = JSON.parse(JSON.stringify(this.initialSequence));
          } catch (all) {
          ...
          try {
            this.initialSequence = JSON.parse(JSON.stringify(this.workflowData.workflowSequence));
          } catch (all) {
        ...
        this.workflowData.workflowSequenceTemp = [];
        if (this.workflowData.workflowSequence !== undefined && this.workflowData.workflowSequence.length > 0) {
          let i = 0;
          let indexI = 0;
          for (const pipe of this.workflowData.workflowSequence) {
            this.workflowData.workflowSequenceTemp.push({
      ...
      for (const appDetails of pipe.applicationDetails) {
        ...
        this.workflowData.workflowSequenceTemp[indexI].applicationDetails.push({});
        appDetails.allPipelineNames = [];
        const c = 0;
        for (const pipeDetails of appDetails.pipelineDetails) {
            appDetails.applicationName = pipeDetails.applicationName;
            this.getPipelineListCopyEdit(pipeDetails.applicationName, pipeDetails.pipelineName, indexI, indexJ);
             this.workflowData.workflowSequenceTemp[indexI].applicationDetails[indexJ].applicationName = pipeDetails.applicationName;
            this.workflowData.workflowSequenceTemp[indexI].appPipeNamesDetails = true;
            i++;
          }
        indexJ++;
      }
    ...
    this.Idprestapi.getPipelineListforWorkflow(appName).then(response => {
      ...
      if (resp.resource !== "No Pipelines") {
        ...
        for (const i of pipData.names) {
          if (pipeName === i) {
        this.workflowData.workflowSequence[indexI].applicationDetails[indexJ].allPipelineNames.push({ "id": c, "itemName": pipeName});
      }
  ...
  togglePanel(i) {
    if (this.workflowData.workflowSequenceTemp[i].expand === undefined || !this.workflowData.workflowSequenceTemp[i].expand) {
  ...
  removeSequence(index) {
    const x = confirm("Are you sure you want to delete the Step No_" + (index + 1) + "?");
    if (x) {
  ...
  removeApplication(i, j) {
    const x = confirm("Are you sure you want to remove application: " +
  ...
  getApplicationNames(data) {
    this.Idprestapi.getFilteredApplicationNames(data)
      .then(response => {
        try {
  ...
  refreshSaveDetails(i, j) {
    if (this.workflowData.workflowSequenceTemp[i].applicationDetails[j].applicationName !== undefined &&
      this.workflowData.workflowSequenceTemp[i].applicationDetails[j].applicationName !== ""  ) {
      let mesg = "Do you want to reset the pipeline name ";
      mesg = mesg + this.workflowData.workflowSequence[i].applicationDetails[j].applicationName;
      const con = confirm(mesg);
      if (con) {
        const applicationName = this.workflowData.workflowSequence[i].applicationDetails[j].applicationName;
  ...
  getComponentToLoad(i, j, k) {
    ...
    let componentRef;
      componentFactory = this.componentFactoryResolver.resolveComponentFactory(TriggerComponent);
      viewContainerRef = this.dynamicComponent.viewContainerRef;
      viewContainerRef.clear();
      componentRef = viewContainerRef.createComponent(componentFactory);
      this.workflowData.workflowSequenceTemp[i].componentInstance = (<TriggerComponent>componentRef.instance);
  ...
  getTriggerDetails(i, j, k) {
    this.Idpdata.appName = this.workflowData.workflowSequence[i].applicationDetails[j].applicationName;
    this.Idpdata.pipelineName = this.workflowData.workflowSequence[i].applicationDetails[j].pipelineDetails[k].pipelineName;
  ...
  deleteSequenceDetails(i, j) {
    ...
    const pipeName = this.workflowData.workflowSequence[i].pipelineDetails[j].pipelineName;
  ...
  validate() {
    ...
    for (const appArray of this.workflowData.workflowSequence) {
     ...
     for (const pipe of pipeArray.pipelineDetails) {
      if (pipe.applicationName !== undefined &&
  ...
  submit() {
    ...
    if (this.Idpdata.workflowTrigger) {
      ...
      if (this.Idpdata.allFormStatus.basicInfo &&
        ...
        if (!this.Idpdata.allFormStatus.workflowInfo && this.listToFillFields.indexOf("workflowInfo") === -1 ) {
  ...
  submitData() {
    ...
    this.Idpdata.data.masterJson["pipelines"] = this.workflowData.workflowSequence;

    let data = this.Idpdata.data.masterJson;
    console.log(data);
    data = this.idpencryption.encryptAES(JSON.stringify(data));
    this.Idprestapi.submit(data)
        .then(response => {
          try {
            ...
            if (errorMsg === null && resp.resource.toLowerCase() === "success") {
              ...
              if (this.formStatusObject.operation !== "edit") {
                const actiondata = {
                  "applicationName": this.Idpdata.data.masterJson.basicInfo.applicationName,
                  "method": "create",
  ...
  getAppDetails() {
    this.Idprestapi.getApplicationDetails(this.Idpdata.data.masterJson.basicInfo.applicationName)
      .then(response => {
        if (response) {
  ...
  triggerData() {
    jQuery("#triggerAlertForthis").modal("hide");
      this.loader = "on";
          ...
          this.Idprestapi.triggerJobs(requestData)
            .then(response => {
              try {
              if (response) {
  ...
  onItemSelect(item: any, i, j) {
    ...
    if (!this.workflowData.workflowSequence[i].applicationDetails[j].pipelineDetails) {
      this.workflowData.workflowSequence[i].applicationDetails[j].pipelineDetails = [];
    }
    this.workflowData.workflowSequence[i].applicationDetails[j].pipelineDetails.push({pipelineName: item.itemName});
  }
  OnItemDeSelect(item: any, i, j) {
  const index = this.workflowData.workflowSequence[i].applicationDetails[j].
  pipelineDetails.findIndex(pipeline => pipeline.pipelineName === item.itemName);
  this.workflowData.workflowSequence[i].applicationDetails[j].pipelineDetails.splice(index, 1);
  }
  onSelectAll(items: any, i, j) {
    ...
    const index = this.workflowData.workflowSequence[i].
    applicationDetails[j].pipelineDetails.findIndex(pipeline => pipeline.pipelineName === item.itemName);
      if (index === -1) {
     this.workflowData.workflowSequence[i].applicationDetails[j].pipelineDetails.push({pipelineName: item.itemName});
    }
 }
  ...
  const x = confirm("Are you sure you want to reset all the pipeline names for " +
   this.workflowData.workflowSequence[i].applicationDetails[j].applicationName + " ?");
 if (x) {
    this.workflowData.workflowSequence[i].applicationDetails[j].pipelineDetails = [];
 }

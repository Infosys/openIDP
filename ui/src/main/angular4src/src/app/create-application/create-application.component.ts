...
export class CreateApplicationComponent implements OnInit {
  ...
  getAppInfo() {
    ...
    this.IdprestapiService.getOrganizationWiseApplicationNames().then(
      (response) => {
        try {
          if (response) {
  ...
  removeEnvOwnerConfirm(modalRef) {
    this.grantAccess.environmentOwnerDetails.splice(
      modalRef.content.envIndex,
      1
    );
    this.tempObjectApp.environmentOwnerDetails.splice(
      modalRef.content.envIndex,
      1
    );
    modalRef.hide();
  ...
  removeEnvProvConfirm(modalRef) {
    this.grantAccess.environmentProvDetails.splice(
      modalRef.content.envIndex,
      1
    );
    this.tempObjectApp.environmentProvDetails.splice(
      modalRef.content.envIndex,
      1
    );
    modalRef.hide();
  ...
  removeSlave(index) {
    ...
    this.modelForRemoveSlaveRef = this.modalService.show(
      this.modalforRemoveSlave
    );
  ...
  checkEnvName(index) {
    for (let i = 0; i < this.grantAccess.environmentOwnerDetails.length; i++) {
      if (
        i !== index &&
        this.grantAccess.environmentOwnerDetails[i].environmentName !==
          undefined &&
        this.grantAccess.environmentOwnerDetails[index].environmentName !==
          undefined &&
        this.grantAccess.environmentOwnerDetails[
          i
  ...
  checkEnvProvName(index) {
    for (let i = 0; i < this.grantAccess.environmentProvDetails.length; i++) {
      if (
        i !== index &&
        this.grantAccess.environmentProvDetails[i].environmentProvName !==
          undefined &&
        this.grantAccess.environmentProvDetails[index].environmentProvName !==
          undefined &&
        this.grantAccess.environmentProvDetails[
          i
        ].environmentProvName.toLowerCase() ===
          this.grantAccess.environmentProvDetails[
            index
          ].environmentProvName.toLowerCase()
      ) {
        this.tempObjectApp.environmentProvDetails[index].messageEnvName =
          "Environment Prov name should be unique.";
        break;
      } else {
        this.tempObjectApp.environmentProvDetails[index].messageEnvName = "";
      }
  ...
  checkSlaveName(index) {
    for (let i = 0; i < this.grantAccess.slavesDetails.length; i++) {
      if (
        i !== index &&
        this.grantAccess.slavesDetails[i].slaveName !== undefined &&
        this.grantAccess.slavesDetails[index].slaveName !== undefined &&
        this.grantAccess.slavesDetails[i].slaveName.toLowerCase() ===
          this.grantAccess.slavesDetails[index].slaveName.toLowerCase()
      ) {
        this.tempObjectApp.slavesDetails[index].messageSlvName =
          "Slave name should be unique.";
        break;
      } else {
        this.tempObjectApp.slavesDetails[index].messageSlvName = "";
      }
  ...
  checkSlaveStage() {
    for (let i = 0; i < this.grantAccess.slavesDetails.length; i++) {
      if (
        (this.grantAccess.slavesDetails[i].build === undefined ||
          this.grantAccess.slavesDetails[i].build === "off") &&
        (this.grantAccess.slavesDetails[i].deploy === undefined ||
          this.grantAccess.slavesDetails[i].deploy === "off") &&
        (this.grantAccess.slavesDetails[i].test === undefined ||
          this.grantAccess.slavesDetails[i].test === "off")
      ) {
  ...
  getApplicationDetailedInfo(data) {
    ...
    this.IdprestapiService.getApplicationInfo(data1).then((response) => {
      try {
        ...
        if (
          this.grantAccess.checkMarxDetails === undefined ||
          this.grantAccess.checkMarxDetails === null
        ) {
          this.grantAccess.checkMarxDetails = {
            checkmarxBtn: "",
            checkmarxUrl: "",
        ...
        if (
          this.FlagEdit &&
          this.grantAccess.environmentProvDetails !== undefined &&
          this.grantAccess.environmentProvDetails.length >= 0
        ) {
          this.envProvPanelExpand = true;
        }
        if (
          (this.FlagEdit && this.grantAccess.artifactToStage === undefined) ||
  ...
  checkCheckBox() {
    for (let i = 0; i < this.grantAccess.slavesDetails.length; i++) {
      if (
        this.grantAccess.slavesDetails[i].workspacePath !== undefined &&
    ...
    this.IdprestapiService.getExistingAppNames().then((response) => {
      try {
        ...
        for (let i = 0; i < appDetails.applications.length; i++) {
          ...
          if (
            app !== undefined &&
            this.grantAccess.applicationName !== undefined &&
  ...
  go() {
    ...
    if (this.FlagEdit === true) {
      ...
      if (this.checkSlaveStageOnGo()) {
        this.IdprestapiService.editApplicationDetails(this.grantAccess).then(
          (response) => {
            try {
        ...
        this.slaveStageModalRef = this.modalService.show(
          this.modalforSlaveStageSelection
        );
        this.grantloading = false;
      }
    } else {
      if (this.flagNew === true) {
        if (this.checkSlaveStageOnGo()) {
          ...
          this.IdprestapiService.createApplication(this.grantAccess).then(
            (response) => {
              try {
                this.grantloading = false;
                if (
                    ...
                    this.IdprestapiService.getUserName().then((response) => {
                      try {
                        if (response) {
                          ...
                          for (
                            let i = 0;
                            i < this.IdpdataService.permissions.length;
                            i++
                          ) {

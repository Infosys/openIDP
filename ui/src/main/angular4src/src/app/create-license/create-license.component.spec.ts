...
PagePersmission: any= {
 ...
 data: any =  JSON.parse(JSON.stringify(this.template));
  ...
  const mockResponseSubDetail = {
    resource: JSON.stringify({"status": {"licenses": ["idp"]}})
  };
...
class  IDPEncryptionStub {
  ...
  class SubscriptionServiceStub {
    getAllSubscriptions() {
      return Promise.resolve(new Response(
        new ResponseOptions({body: mockResponseSubDetail, "status": 1})
  ));
    }
  }
  class RouterStub {
  ...
  beforeEach(async(() => {
    TestBed.configureTestingModule({
      declarations: [ CreateLicenseComponent, ParentFormConnectComponent ],
  ...
  it("should run #userHaveLicences()", async(() => {
    const result = component.userHaveLicences();
  }));

  it("should run #getstatus()", async(() => {
     const result = component.getstatus("10/12/2018");
  }));

  it("should run #clearExpiryDate()", async(() => {
     const result = component.clearExpiryDate();
  }));

  it("should run #onItemSelect()", async(() => {
    const result = component.onItemSelect("string");
  }));

  it("should run #OnItemDeSelect()", async(() => {
     const result = component.OnItemDeSelect("item");
  }));

  it("should run #onSelectAll()", async(() => {
     const result = component.onSelectAll("items");
  }));

  it("should run #onDeSelectAll()", async(() => {
    const result = component.onDeSelectAll("items");
  }));

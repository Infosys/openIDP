...
export class BasicInfoComponent implements OnInit {
  ...
  constructorFunction() {
    ...
    if (
      this.basicInfo.masterSequence !== undefined &&
      this.basicInfo.masterSequence === "workflow"
    ) {
  ...
  addBuildInterval() {
    this.basicInfo.buildInterval.event.push({
      type: "--Select--",
      hour: "00",
      minute: "00",
    });
  ...
  init() {
    ...
    this.groupIds = [
      { groupName: "abc", groupId: "g1" },
      { groupName: "abfdhc", groupId: "gfh1" },
    ];
    this.osList = [
      {
        name: "Windows",
      ...
      if (response) {
        ...
        try {
          ...
          if (parsed) {
            ...
            this.Idpdata.application = this.idpencryption.doubleEncryptApplicationDetailsPassword(
              this.Idpdata.application
            );
            const application = this.Idpdata.application;
            if (
              !(
                application.artifactToStage === undefined ||
  ...
  getPipelineNames(appName) {
    this.IdprestapiService.getJobs(appName).then((response) => {
      try {
        // console.log('appName ' + appName);
        const pipetemp = JSON.parse(
          this.idpencryption.decryptAES(response.json().resource).toLowerCase()
        );
        if (typeof pipetemp !== "string") {
          this.pipeLineNames = JSON.parse(
            this.idpencryption.decryptAES(response.json().resource)
          );
          if (
            this.basicInfo.pipelineName !== undefined &&
            this.basicInfo.pipelineName !== null &&
            this.basicInfo.pipelineName !== ""
          ) {
  ...
  checkJobName(pipeName) {
    if (this.pipeLineNames) {
      for (let i = 0; i < this.pipeLineNames.pipelines.length; i++) {
        if (
          pipeName !== undefined &&
          this.pipeLineNames.pipelines[i].pipelineName !== undefined &&
          pipeName.toLowerCase() ===
            this.pipeLineNames.pipelines[i].pipelineName.toLowerCase() &&
  ...
  getApplicationName(data) {
    ...
    this.IdprestapiService.checkApplicationNames(data, oprMethod).then(
      (response) => {
        try {
  ...
  checkCheckBox() {
    if (
      (this.Idpdata.data.basicInfo.buildInterval.buildIntervalValue >= 0 &&
        this.Idpdata.data.basicInfo.buildInterval.buildInterval !== "") ||
      (this.Idpdata.data.basicInfo.buildInterval.event !== undefined &&
        this.Idpdata.data.basicInfo.buildInterval.event[0].type !==
          "--Select--")
    ) {
  ...
  copyEditOperation() {
    ...
    this.IdprestapiService.getPipelineDetails(this.data).then((response) => {
      //console.log(new Date().toUTCString(), "Pipeline details retrieved");
      try {
        this.responseData = this.idpencryption.decryptAES(
          response.json().resource
        );
        const resp = JSON.parse(this.responseData);
        if (
          resp.pipelineJson.basicInfo.masterSequence === undefined ||
          resp.pipelineJson.basicInfo.masterSequence === "pipeline"
        ) {
          resp.pipelineJson = this.idpencryption.doubleEncryptPassword(
            resp.pipelineJson
          );
          resp.pipelineJson.basicInfo.masterSequence = "pipeline";
        }
        ...
        if (
          this.basicInfo.jiraProjectKey === undefined ||
          this.basicInfo.jiraProjectKey === ""
        ) {
        ...
        if (
          this.Idpdata.data.buildInfo.buildtool !== undefined &&
        ...
        if (
          this.Idpdata.data.buildInfo.artifactToStage !== undefined &&
          this.Idpdata.data.buildInfo.artifactToStage.artifact !== undefined &&
        ...
        // console.log("Err msg" + e);console.log("Err msg" + e);
      }
    });
    // Fetches Pipeline List for edit/copy operation
    this.IdprestapiService.getPipelineList(this.applicationName).then(
      (response) => {
        try {
          //console.log(response);
          ...
          const pipData = JSON.parse(
            this.idpencryption.decryptAES(resp.resource)
          );
          let c = 1;
          for (const i of pipData.pipelines) {
            this.Idpdata.data.pipelineList.push({
              id: c,
              itemName: i.pipelineName,
            });
            c = c + 1;
          }
        } catch (e) {
          //console.log(e);
          console.log("Failed to get PipelineList:", e);
        }
      }
    );
    this.IdprestapiService.getPipelinePermission(this.data).then((response) => {
      this.responseData = this.idpencryption.decryptAES(
        response.json().resource
      );
      const resp = JSON.parse(this.responseData);
          ...
          if (response) {
            if (
              ...
              if (JSON.parse(temp).names.length !== 0) {
                ...
                alert(
                  "Variants are not available for application " +
  ...
  getLandscapeNamesForSap() {
    ...
    this.IdprestapiService.getLandscapesForSap(applicationName).then(
      (response) => {
        try {
          if (response) {
            if (
  ...
  getAppmartDetails() {
    ...
    if (this.Idpdata.isRmsApp === true) {
      ...
      this.IdprestapiService.getRmsApplicationName(
      ...
      this.IdprestapiService.getAppmartDetails(this.Idpdata.idpUserName).then(
        (response) => {
          try {
            if (response) {
              if (
  ...
  selectedApp() {
    ...
    if (
      this.basicInfo.applicationName !== undefined &&
      this.basicInfo.applicationName !== "" &&
  ...
  resetData() {
    ...
    if (x) {
      ...
      if (
        this.Idpdata.data.formStatus.operation !== "edit" &&
  ...
  removeCustomAdmin(index) {
    // add condition
    const adminName =
      this.basicInfo.customPipelineAdmins[index].adminName === undefined
        ? "entry"
        : "'" + this.basicInfo.customPipelineAdmins[index].adminName + "'";
    const x = confirm(
      "Are you sure to remove " + adminName + " as custom pipeline admin?"
    );
    if (x) {
  ...
  checkEnvNamesForCopyEdit(applicationName) {
    this.IdprestapiService.getEnvironmentNames(applicationName).then(
      (response) => {
        try {
          if (
            response.json().errorMessage === null &&
            ...
            if (this.env.length !== 0) {
              if (
                this.testInfo.testEnv !== undefined &&
                this.testInfo.testEnv.length !== 0
              ) {
                for (
                  let j = this.testInfo.testEnv.length;
                  j < this.env.length;
                  j++
                ) {
              ...
              if (
                this.deployInfo.deployEnv !== undefined &&
                this.deployInfo.deployEnv.length !== 0
              ) {
                for (
                  let i = this.deployInfo.deployEnv.length;
                  i < this.env.length;
                  i++
                ) {
  ...
  getPipelineAdminNames(appName, pipelineAdminName, j) {
    this.IdprestapiService.getPipelineAdmins(appName).then((response) => {
      try {
        const pipetemp = JSON.parse(
          this.idpencryption.decryptAES(response.json().resource).toLowerCase()
        );
        if (typeof pipetemp !== "string") {
          this.pipeLineAdminNames = JSON.parse(
            this.idpencryption.decryptAES(response.json().resource)
          );
          if (this.pipeLineAdminNames) {
            for (let i = 0; i < this.pipeLineAdminNames.length; i++) {
              if (
                pipelineAdminName !== undefined &&
                this.pipeLineAdminNames[i] !== undefined &&
                pipelineAdminName.toLowerCase() ===
                  this.pipeLineAdminNames[i].toLowerCase()
              ) {
  ...
  isCPASelectedOnceAllUser(isFormValid: boolean) {
    ...
    if (this.basicInfo !== undefined && this.basicInfo !== null) {
      if (
        this.basicInfo.customPipelineAdmins !== undefined &&
        this.basicInfo.customPipelineAdmins !== null
      ) {
        for (let i = 0; i < this.basicInfo.customPipelineAdmins.length; i++) {
          const isSelected: boolean = this.isCPASelectedOnce(
            this.basicInfo.customPipelineAdmins[i]
          );
          if (!isSelected) {
            return false;

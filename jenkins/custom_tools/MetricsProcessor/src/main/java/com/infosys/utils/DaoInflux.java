...
/**
 * the class daoinflux has methods for compairing used/unused json files and send the data from unsued into database
...
public class DaoInflux {
	...
	/**
	 * method to process unused files
	 * @param appName
	 * @param pipeline
	 * @param jsonFileLocation
	 * @param postgresServiceURL
	 * @param pgsunames
	 * @param pgspwd
	 * @throws IOException
	 */
	public void readmethod(String appName, String pipeline, String jsonFileLocation,
			 String postgresServiceURL,String pgsunames, String pgspwd) throws IOException	{
		...
		for (String filename : fileNames) {
			if (!usedJsonFiles.contains(filename))
			{
				try
				{
						...
						boolean value=p.postJSON(filename, postgresServiceURL,appName,pipeline,temp1,pgsunames,pgspwd);
						if(value)System.out.println("Json Posted to desired service");

...
public class ConvertDSPriv {
	...
	/**
	 * 
	 * Method to parse dspriv reports
	 * @param inputPath
	 * @param json
	 * @return
	 */
	public static CoverageDSPrivJson  convert(String inputPath,JsonClass json) 
	{
		...
		try {
			...
			CoverageDSPriv c = (CoverageDSPriv) jaxbUnmarshaller
					.unmarshal(file);
			...
			for(CoverageDSPriv.Module.NamespaceTable.Class cd2:cd1)
			{
				...
				temp.setLineCoverage(Double.toString((double)(cd2.getLinesCovered())/((double)(cd2.getLinesCovered())+(double)(cd2.getLinesNotCovered())+(double)(cd2.getLinesPartiallyCovered()))));

...
public class EntryHome {
	...
	/**
	 * method to delete report file from workspace
	 * @param path
	 */
	...
	/**
	 * method to go through all reports, generate single jsonclass object and finally store in the workspace
	 * @param args
	 * @throws IOException
	 */
	public static void convertAllFiles(String [] args) throws IOException {
		...
		if(args.length > 9 && !args[9].equals("NA"))buildTool=args[9];
		if(args.length > 10 && !args[10].equals("NA"))sapWarName=args[10];
		if(args.length > 19 && args[19].equalsIgnoreCase("cast"))isCast=true;
	...
	if(args.length > 20 && args[20]!=null &&  !args[20].equals("NA")&& args[20].equalsIgnoreCase("story")){
		...
		if (args.length > 20 && args[20] != null && !args[20].equals("NA")
				&& args[20].equalsIgnoreCase("filenet")) {
		...
		//If VSTS is selected
				...
				for (int i = 0; i < list2.size(); i++) {
					if (list2.get(i).isFile() && list2.get(i).getName().toLowerCase().contains(args[1].toLowerCase())) {
						...
						json = computeIDPJson(list2.get(i), json, args, ca, tr,  coverageDetails,
								serviceUrl,jobName,jenkinsServer,pipelineId,stage,artifactName,jenkinsUserName,jenkinsPassword,siebel);				}
				}
	...
	/**
	 * method to add report urls in the jsonclass object
	 * @param args
	 * @param json
	 */
	private static void searchReports(String[] args, JsonClass json) {
		...
		fileNamesToSearch.add("report-jshint-checkstyle.html");//jshint checkstyle		
...
//		
		...
		for(File x :reportsList)
		{
			...
			// removing last slash
			finalURL = (finalURL.lastIndexOf('/')!=-1)?finalURL.substring(0, finalURL.lastIndexOf('/')):finalURL;
			...
			else if (finalURL.toLowerCase().toLowerCase().contains("coverage.xml"))
			{
				...
				if(validateUrl(locuptobn+"/cobertura") && (finalURL.toLowerCase().toLowerCase().contains("cobertura"))){
			...
			else if (finalURL.contains("index.html") && (finalURL.contains("test-output") && validateUrl(json.getBaseURL()+"/"+finalURL)))
			{
			...
			if (r.getAccelerateReports()!=null || r.getCheckmarxReports()!=null 
					|| r.getCheckReports()!=null 
							||r.getCoberturareports()!=null
									||r.getFindBugsReports() !=null || r.getiFastReports()!=null || r.getiTopsReps() !=null || r.getjMeterReports() !=null
											||r.getJunitReports() !=null ||r.getPMDReports()!=null
													||r.getQualitia() !=null ||r.getRobotReports() !=null || 
					r.getSeleniumReports()!=null)
			{
			json.setReports(r);
			}
		}
	...
	/**
	 * method to validate url of reports
	 * @param url1
	 * @return
	 */
private static boolean validateUrl(String url1)
{
	...
	/**
	 * generates fileoutput path for the jsonclass object
	 * @param args
	 * @param json
	 * @param timeStamp
	 * @return
	...
	/**
	 * creates output directory for the jsonclass object file
	 * @param path
	 * @return
	 */
	...
	/**
	 * returns jsonclass type object after parsing each of the report files
	 * @param reportfile
	 * @param json
	 * @param args
	 * @param ca
	...
	private static JsonClass computeIDPJson(File reportfile, JsonClass json, String [] args, List<CodeAnalysis> ca,
			List<TestCaseResult> tr, List <CoverageDetails> cd,String serviceUrl,String jobName,String jenkinsServer,String pipelineId,String stage,String artifactName,String jenkinsUserName,String jenkinsPassword,Siebel siebel) throws IOException {
		String idPrefixFlag = args[4];
		String appName = args[1];
		...
		if (reportfile.getName().toLowerCase().endsWith("txt") || reportfile.getName().toLowerCase().endsWith("xml")
			...
			if((reportfile.getName().toLowerCase().contains("pqm_report_pmd-existing")) && reportfile.getName().toLowerCase().endsWith(".txt"))
			{
			...
			else if((reportfile.getName().toLowerCase().contains("_repo_") || reportfile.getName().toLowerCase().contains("_static_")) && reportfile.getName().toLowerCase().endsWith("csv"))
			{
			...
			else if(reportType.toLowerCase().contains("parasoftsoatest") && reportType.toLowerCase().endsWith("xml"))
			{
			...
			}else if (reportfile.getName().toLowerCase().contains("generalreport")&&reportfile.getName().toLowerCase().endsWith("xml"))
			{
				try {
			...
			else if(reportfile.getName().toLowerCase().contains("pythontest")&&reportfile.getName().toLowerCase().endsWith("xml"))
			{
				try
				{
			...
			else if (reportfile.getName().toLowerCase().contains("result")&&reportfile.getName().toLowerCase().endsWith("txt")) 
			{ 
				try{
			...
			else if(reportfile.getName().toLowerCase().contains("lintreport") && reportfile.getName().toLowerCase().endsWith("xml"))
			{
				Lint lint=new Lint();
				...
				Map<String, String> ruleToValueCheckStyle = RecommendationByCSV.createMap("csv/checkstyle.csv", "Check");
			...
			else if(reportfile.getName().toLowerCase().contains("buildlog"))
{
				...
				json.setCodeQuality(codeQuality);
				json.setCodeAnalysis(ca);
			} else if ((reportType.toLowerCase().contains("junit_")&& reportType.toLowerCase().contains("lisa"))||(reportType.toLowerCase().contains("junit_")&&!reportType.toLowerCase().contains("junit_test")) || reportType.toLowerCase().contains("android")) {
				//*********
			...
			else if (reportfile.getName().toLowerCase().contains("istanbulcoverage")) 
			{
				...
				if(cc==null)cc=new Codecoverage();
			...
			else if(reportType.toLowerCase().contains("report") && !reportType.contains("modReport_ecatt") && !reportType.contains("SAP_REPORTSconsoleSci") && !reportType.contains("SAP_REPORTSconsoleUnit") && !reportType.toLowerCase().contains("general") ){
				...
				List<TestCaseResult> listTR=json.getTestCaseResult();
				if(listTR==null)listTR=new ArrayList<>();
			...
			else if(reportType.contains("modReport_ecatt"))
			{
				...
				if(tc==null)tc=new ArrayList<>();
				if(fn==null)fn=new Functional();
	...
	private static String generateOpFilename(String[] args, JsonClass json, String timeStamp) {
		String output;
		if (json.getbuildId() != null) {
			output = args[0].replaceAll("\\\\", "/") + "/" + args[1] + "_"+args[5]+"_"+ json.getbuildId() + "_" + timeStamp + JSON;
		} else {
	...
	/**
	 * method to parse changelog report
	 * @param args
	 * @throws IOException
	 */
	public static void convertChangeLog(String [] args) throws IOException {
		...
		int lastIndex = path.lastIndexOf('/') > path.lastIndexOf("\\\\") ? path.lastIndexOf('/') : path.lastIndexOf("\\\\");
		...
		if (p == null) {
			...
			try{
				...
				CSVWriterUtility.writeLine(fw, Arrays.asList("S.No.", "ID", "LastModified", "CommitMessage", "FileVersion"),
						',', '"');
				int i = 0;
	...
	private static JsonClass cbi(File[] list2, JsonClass json, String[] args) throws IOException {
		...
		for (File element : list2) {
			if (element.isFile() && element.getName().toLowerCase().contains(args[1].toLowerCase())
				...
				json = ConvertBuildInfo.convert(element.getCanonicalPath(), json, preparePrefixForId(args[5], args[1]), tfsPath);
				break;
			}
		}
		return json;
		...
		if (folder2.exists() && folder2.isDirectory() && folder2.listFiles().length > 0) {
			...
			for (File element : list1) {
				if (element.isFile() && element.getName().toLowerCase().contains(args2.toLowerCase())
						&& element.getName().toLowerCase().contains("changeset")
						&& element.getName().endsWith("csv")) {
	...
	public static JsonClass addToRuleSet(JsonClass jcObj) {
		...
		for (CodeAnalysis ca : jcObj.getCodeAnalysis()) {
			if (!tempMap.contains(ca.getruleName())) {
				...
				rs.add(getRuleSetObject(ca.getruleName().replaceAll(":|-", "_"), ca.getSeverity(), ca.getMessage(), ca.getLine(), ca.getruleName(),
						ca.getcategory()));
			}
		}
		jcObj.setRuleSet(rs);

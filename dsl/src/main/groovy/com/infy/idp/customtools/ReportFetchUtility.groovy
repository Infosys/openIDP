...
class ReportFetchUtility{
    ...
    */
	...
	public static void invokeTool(context, jsonData) {
		...
		def command;
		command = """java -jar "${data.get('CUSTOM_TOOL_JAR')}" ${data.get('JSERVER')} ${data.get('USERNAME')} ${data.get('PASSWORD')} "${data.get('WORKSPACE_LOC')}" "${data.get('JOBNAME')}" "${data.get('APPNAME')}" pmd:pmd.xml cs:checkstyle-result.xml fb:findbugsXml.xml cob:coverage.xml pqm:pqm_report_pmd.txt test:junit.xml acc:report.json if:Fastest.json js:report-jshint-checkstyle.xml tng:testng-results.xml qlta:SummaryReport.xml rbt:output.xml tslint:result.txt ut:junit_ut.xml istanbul:istanbulcoverage.xml jcc:jacocoCoverage.xml put:pythontest.xml ${data.get('JMETER')}  sui:TEST-*.xml set:modReport_ecatt.txt sut:SAP_REPORTSconsoleUnit.txt sci:SAP_REPORTSconsoleSci.txt fxc:AnalysisResult.xml Junitgo:JUnit_Test.xml alt:lintReport.xml orclanalysis:CodeAnalysisReports.xls oatstest:generalReport.xml trx:*.trx ptr:protractorxml.xml scala:scalastyle-output.xml pst:ParasoftSOATest.xml ser:Repo.csv ses:Static.csv"""

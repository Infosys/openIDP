 ...
 */
class PythonUT extends Specification {
    ...
    void 'Executing Test Conditions on #file.name'(File file) {
        ...
        JobManagement jm = new JenkinsJobManagement(System.out, ['JSON_Input': (new File(env['basePath'] + '\\UTJsons\\Python.json')).text, 'Pipeline_Script_Path': env['dirPath'] + '\\pipeline_sequence'], new File('.'))

 ...
 */
class DotNetUT extends Specification {
    ...
    void 'Executing Test Conditions on #file.name'(File file) {
        ...
        JobManagement jm = new JenkinsJobManagement(System.out, ['JSON_Input': (new File(env['basePath'] + '\\UTJsons\\DotNet.json')).text, 'Pipeline_Script_Path': env['dirPath'] + '\\pipeline_sequence'], new File('.'))
        ...
        items.jobs.each { GeneratedJob generatedJob ->
            ...
            if (jobName.endsWith('Build')) {
                ...
                with(new XmlParser().parse(new StringReader(text))) {
                    ...
                    println "Testing if builders plugin conditionalbuilder"
                    builders.'org.jenkinsci.plugins.conditionalbuildstep.ConditionalBuilder'
                    println "Passed"

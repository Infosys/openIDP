 ...
 */
class JavaAntUT extends Specification {
    ...
    void 'Executing Test Conditions on #file.name'(File file) {
        ...
        JobManagement jm = new JenkinsJobManagement(System.out, ['JSON_Input': (new File(env['basePath'] + '\\UTJsons\\Java_Ant.json')).text, 'Pipeline_Script_Path': env['dirPath'] + '\\pipeline_sequence'], new File('.'))
        ...
        items.jobs.each { GeneratedJob generatedJob ->
            ...
            if (jobName.endsWith('Build')) {
                ...
                with(new XmlParser().parse(new StringReader(text))) {
                    ...
                    println "Testing if buildWrapper plugin copyToSlave"
                    buildWrappers.'com.michelin.cio.hudson.plugins.copytoslave.CopyToSlaveBuildWrapper'
                    println "Passed"

                    println "Testing if publisher Checkstyle"
                    publishers.'hudson.plugins.checkstyle.CheckStylePublisher'
